...
class MainWin:
    ...
    def GuiShow(self):
        ...
        retvalue = self.cvcore.CvInit(console = self.outtext)
        if retvalue < 0:
    ...
    def ShowFrame(self):
        img = self.cvcore.DetectionToShow()
    ...
    def FaceRecognition(self, win):
        dicResult= self.cvcore.FaceRecognition()
        if len(dicResult) <= 0:
            messagebox.showerror(title = 'Error', message = self.cvcore.errorMessage, parent = win)
        else:  
            string  = 'name: ' + dicResult['name'] + '\nconfidence: ' + dicResult['confidence'] 
...
class MenuGui:
    ...
    def GuiShow(self, master, cvcore):
        ...
        self.cvCore.SetParentWidget(self.topWin)
    ...
    def CreateNewFace(self, event = None):
        ...
        if 0 == len(note):
            note = 'null'
        faceInfo = self.cvCore.TrainFromCamera(name, note)
    ...
    def DeleteTableItems(self, deleteAll = False):
        ...
        if retvalue:
            for item in obids:
                self.table.delete(item)
            self.cvCore.DeleteFaces(faceIds)
    ...
    def ChangePermission(self, event = None):
        ...
        if newp != repp:
            messagebox.showerror(title = 'Error', message = 'Repeat permission error' )
        else: 
            retvalue = FileOption().WritePermission(oldp, newp)
            if 0 == retvalue:
                messagebox.showinfo(title = 'Info', message = 'Change permission success!' )
                self.CancelResetPermission()
            elif -1 == retvalue:
                messagebox.showinfo(title = 'Error', message = 'Old permission rrror' )
